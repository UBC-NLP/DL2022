...
def token_encode(examples):
    tokenizer_out = tokenizer(examples['text'], truncation=True,  padding="max_length", add_special_tokens=True, max_length=60)
...
def make_collate(tokenizer, block_size, lazy=False):
    ...
    def lazy_collate(examples):
        ...
        examples_tokens = [[tokenizer.convert_ids_to_tokens(token) for token in x] for x in examples["input_ids"].tolist()]
           
        return examples_tokens, examples["input_ids"], examples["attention_mask"]
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    train_dataloader = DataLoader(
        train_dataset, sampler=train_sampler, num_workers=int(args.num_workers), collate_fn=collate, batch_size=args.train_batch_size, pin_memory=True
    )
    ...
    if args.warmup_steps <= 0:
        args.warmup_steps = t_total * args.warmup_portion
    else: 
    ...
    optimizer = AdamW(optimizer_grouped_parameters, lr=args.learning_rate, eps=args.adam_epsilon, betas=(0.9, 0.999))
    ...
    logger.info("  Warm-up steps = %d", args.warmup_steps)
    ...
    for epoch in train_iterator:
        ...
        if (args.local_rank == -1 or torch.distributed.get_rank() == 0) and args.save_steps > 0:
            checkpoint_prefix = "checkpoint"
            # Save model checkpoint
            output_dir = os.path.join(args.output_dir, "{}-{}".format(checkpoint_prefix, epoch+1+args.inital_epoch))
...
def main():
    ...
    parser.add_argument(
        "--num_workers", default=0, type=int, help="multi-process data loading with the specified number of loader worker processes.."
    )
    ...
    parser.add_argument("--warmup_portion", default=0.1, type=float, help="Linear warmup over total * warmup_portion.")
    ...
    if args.config_name:
        config = AutoConfig.from_pretrained(args.config_name, cache_dir=args.cache_dir, padding="max_length")
    elif args.model_name_or_path:
        config = AutoConfig.from_pretrained(args.model_name_or_path, cache_dir=args.cache_dir,  padding="max_length")
    else:
