...
class DataTrainingArguments:
    ...
    train_file: Optional[str] = field(
        default="mt5_train.json", metadata={"help": "The input training data file (a jsonlines or csv file)."}
    )
    validation_file: Optional[str] = field(
        default="mt5_dev.json",
        metadata={
...
def main():
    ...
    prefix = data_args.source_prefix if data_args.source_prefix is not None else data_args.task + ":"
    ...
    if training_args.do_predict:
        ...
        if trainer.is_world_process_zero():
            ...
            for key in sorted(metrics_formatted.keys()):
                logger.info(f"  {key: <{k_width}} = {metrics_formatted[key]:>{v_width}}")
            save_json(metrics, os.path.join(training_args.output_dir, model_args.model_name_or_path.split("/")[-1] + "test_results.json"))
            all_metrics.update(metrics)
            ...
            if training_args.predict_with_generate:
                ...
                output_test_preds_file = os.path.join(training_args.output_dir,  model_args.model_name_or_path.split("/")[-1] + "_test_preds_seq2seq.txt")
                with open(output_test_preds_file, "w") as writer:
                    writer.write("\n".join(test_preds))

        if training_args.do_eval:
            dev_results = trainer.predict(
                eval_dataset,
            ...
            max_val_samples = data_args.max_val_samples if data_args.max_val_samples is not None else len(eval_dataset)
            metrics["dev_samples"] = min(max_val_samples, len(eval_dataset))
            ...
            if trainer.is_world_process_zero():
                ...
                for key in sorted(metrics_formatted.keys()):
                    logger.info(f"  {key: <{k_width}} = {metrics_formatted[key]:>{v_width}}")
                save_json(metrics, os.path.join(training_args.output_dir, model_args.model_name_or_path.split("/")[-1] + "val_results.json"))
                all_metrics.update(metrics)

                if training_args.predict_with_generate:
                    dev_preds = tokenizer.batch_decode(
                        dev_results.predictions, skip_special_tokens=True, clean_up_tokenization_spaces=True
                    )
                    dev_preds = [pred.strip() for pred in dev_preds]
                    output_val_preds_file = os.path.join(training_args.output_dir, model_args.model_name_or_path.split("/")[-1] + "_val_preds_seq2seq.txt")
                    with open(output_val_preds_file, "w") as writer:
                        writer.write("\n".join(dev_preds))
